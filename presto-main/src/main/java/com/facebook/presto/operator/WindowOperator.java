        ...
        implements Operator
{
    ...
    private static class PagesIndexWithHashStrategies
    {
                ...
                List<Integer> sortChannels)
        {
            ...
            this.preGroupedPartitionChannels = preGroupedPartitionChannels.stream()
                    .mapToInt(i -> i)
                    .toArray();
        }
    }
            ...
            implements Transformation<Page, PagesIndexWithHashStrategies>
    {
        ...
        {
            localMemoryContext.setBytes(pagesIndexWithHashStrategies.pagesIndex.getEstimatedSize().toBytes());
        }
    }

    private WorkProcessor<WindowPartition> produceWindowPartitions(PagesIndexWithHashStrategies pagesIndexWithHashStrategies)
    {
            ...
            implements Transformation<Page, WorkProcessor<PagesIndexWithHashStrategies>>
    {
        ...
        void finishRevokeMemory()
        {
            ...
            // PagesIndex overhead memory is not revocable
        ...
        void updateMemoryUsage(boolean revocablePagesIndex)
        {
            ...
            long nonRevocableBytes = currentSpillGroupRowPage.map(Page::getRetainedSizeInBytes).orElse(0L);

            if (revocablePagesIndex) {
                localRevocableMemoryContext.setBytes(pagesIndexBytes);
                localUserMemoryContext.setBytes(nonRevocableBytes);
            }
            else {
                localRevocableMemoryContext.setBytes(0L);
                localUserMemoryContext.setBytes(nonRevocableBytes + pagesIndexBytes);
            }
        }
    }
