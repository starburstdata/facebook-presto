...
public final class TimestampOperators
{
    ...
    public static long castFromSlice(ConnectorSession session, @SqlType("varchar(x)") Slice value)
    {
        if (session.isLegacyTimestamp()) {
            try {
                // Despite the name this accepts literals with and without time zone
            ...
            try {
                ...
                try {
                    timestampWithTimeZone = parseTimestampWithTimeZone(session.getTimeZoneKey(), trim(value).toStringUtf8());
                }
                catch (IllegalArgumentException ei) {
                    throw new PrestoException(INVALID_CAST_ARGUMENT, "Value cannot be cast to timestamp: " + value.toStringUtf8(), ei);
                }
                ISOChronology chronology = getChronology(unpackZoneKey(timestampWithTimeZone));
                return unpackMillisUtc(timestampWithTimeZone) + chronology.getZone().getOffset(unpackMillisUtc(timestampWithTimeZone));
            }
        }
    }
