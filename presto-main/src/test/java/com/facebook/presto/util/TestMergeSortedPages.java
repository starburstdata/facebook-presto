...
public class TestMergeSortedPages
{
            ...
            throws Exception
    {
        ...
        ContinuousWork<Page> mergedPages = MergeSortedPages.mergeSortedPages(
                ImmutableList.of(testPageProducer(rowPagesBuilder(types)
                        .row(1)
                        .build())),
                new SimplePageWithPositionComparator(types, ImmutableList.of(0), ImmutableList.of(DESC_NULLS_LAST)),
                ImmutableList.of(0),
                types,
                newSimpleAggregatedMemoryContext().newLocalMemoryContext(),
                yieldSignal);
            ...
            throws Exception
    {
        List<ContinuousWork<Page>> pageProducers = sortedPages.stream()
                .map(TestMergeSortedPages::testPageProducer)
    ...
    private static ContinuousWork<Page> testPageProducer(List<Page> pages)
    {
        Iterator<Page> pagesIterator = pages.iterator();
        return ContinuousWork.create((Supplier<WorkState<Page>>) () -> {
            if (!pagesIterator.hasNext()) {
                return WorkState.finished();
            }

            return WorkState.ofResult(pagesIterator.next());
        });
    }
}
