        ...
        implements Matcher
{
    ...
    public SortMatcher(ImmutableMap<String, SortOrder> orderingScheme)
    {
    ...
    public MatchResult detailMatches(PlanNode node, PlanNodeStatsEstimate stats, Session session, Metadata metadata, SymbolAliases symbolAliases)
    {
        ...
        SortNode exchangeNode = (SortNode) node;

        for (String alias : orderingScheme.keySet()) {
            Symbol symbol = Symbol.from(symbolAliases.get(alias));
            OrderingScheme nodeOrderingScheme = exchangeNode.getOrderingScheme();
