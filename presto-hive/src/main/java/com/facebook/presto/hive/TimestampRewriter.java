...
public class TimestampRewriter
{
    ...
    private Block modifyTimestampsInArrayBlock(Type type, Block block, LongUnaryOperator modification)
    {
        if (block instanceof AbstractArrayBlock) {
            ...
            Block innerBlock = modifyTimestampsInBlock(arrayBlock.getRawElementBlock(),
                    type.getTypeParameters().get(0),
                    modification);

            return ArrayBlock.fromElementBlock(arrayBlock.getPositionCount(), Optional.ofNullable(arrayBlock.getValueIsNull()), arrayBlock.getOffsets(), innerBlock);
        }
        else {
            ...
            BlockBuilder builder = type.createBlockBuilder(new PageBuilderStatus().createBlockBuilderStatus(), block.getPositionCount());
            for (int i = 0; i < block.getPositionCount(); ++i) {
                if (block.isNull(i)) {
                    builder.appendNull();
                }
    ...
    private Block modifyTimestampsInMapBlock(Type type, Block block, LongUnaryOperator modification)
    {
        if (block instanceof AbstractMapBlock) {
            ...
            Block innerKeyBlock = modifyTimestampsInBlock(mapBlock.getRawKeyBlock(), keyType, modification);
            Block innerValueBlock = modifyTimestampsInBlock(mapBlock.getRawValueBlock(), valueType, modification);

            return mapType.createBlockFromKeyValue(mapBlock.getPositionCount(), Optional.ofNullable(mapBlock.getMapIsNull()), mapBlock.getOffsets(), innerKeyBlock, innerValueBlock);
        }
    ...
    private Block modifyTimestampsInRowBlock(Type type, Block block, LongUnaryOperator modification)
    {
        if (block instanceof RowBlock) {
            ...
            Block[] wrappedFieldBlocks = new Block[rowBlock.getRawFieldBlocks().length];
            for (int i = 0; i < wrappedFieldBlocks.length; i++) {
                wrappedFieldBlocks[i] = modifyTimestampsInBlock(rowBlock.getRawFieldBlocks()[i], type.getTypeParameters().get(i), modification);
            }
            return RowBlock.createRowBlockInternal(rowBlock.getOffsetBase(), rowBlock.getPositionCount(), rowBlock.getRowIsNull(), rowBlock.getFieldBlockOffsets(), wrappedFieldBlocks);
        }
